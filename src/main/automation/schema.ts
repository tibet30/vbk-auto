...
export function automationBlockers(product: Record<string, unknown>) {
  ...
  const cover = presentation?.cover as Record<string, unknown> | undefined;
  if (!presentation || cover?.source !== "ctripLibrary" || !cover.poi) {
    blockers.push({
      label: "产品图文封面",
      detail: "请配置携程图库封面的 POI 与图片要求；自动录入只使用携程真实图库图片。",
    });
  }
  const commercial = product.commercial as Record<string, unknown> | undefined;
  if (!commercial) {
    blockers.push({ label: "套餐与价格", detail: "请先生成套餐名称和景点票价估算。" });
  } else {
    if (!commercial.packageName) blockers.push({ label: "套餐名称", detail: "请补充套餐名称。" });
    if (!commercial.pricing) blockers.push({ label: "价格", detail: "请按行程景点票价生成成人价、儿童价与最低成团人数。" });
  }
