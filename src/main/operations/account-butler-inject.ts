 ...
 *
 * 用途：
 *  - 新建产品时：若当前 VBK 已登录、账号已配管家、且 product.operations.bookingControls.butler
...
import type { ContactCardSelection, CreateProductInput, ProductDetail } from "../../shared/contracts.js";
...
export interface CreateProductWithAccountButlerResult {
...
export function injectAccountButler(
  ...
  if (!product) return { written: false, reason: "产品不存在" };
  ...
  try {
    next = applyManualReviewField(product.product, { field: "butlerContact", selection: raw });
  } catch (error) {
