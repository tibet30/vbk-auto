 ...
 * detach 时整条 run 标 failed + 项目 blocked。automation:retry 因无
 ...
 *      project.status=draft_saved, screenshot 重新尝试（warn-only）。
...
async function setupStuckProject(db: VbkDatabase, run: AutomationRun) {
  const project = db.createProject({ destination: "太原", days: 2, productForm: "privateTour" });
  db.setProductId(project.id, "P-Ctrip-1");
  db.updateProduct(project.id, project.product, "blocked");
  db.saveAutomation(project.id, run);
  return project.id;
}

// ───────────────────────── L1: 命中 ─────────────────────────

test("L1 命中：业务全 completed + 截图错误 log → run=succeeded + 项目=draft_saved + 重新截图", async (t) => {
  const db = await freshDb(t);
  const projectId = await setupStuckProject(db, makeStuckRun());
  ...
  const recovered = await recoverLegacyScreenshotFalseFailure(ctx, projectId, lock);
  assert.equal(recovered, true, "命中后必须返回 true");

  const after = db.getProject(projectId)!;
  ...
  assert.ok(emitCalls.includes(projectId));
  // 日志里有说明性 warning
...
test("L1 命中后 logs 末尾追加 warning 但旧 logs 全部保留", async (t) => {
  ...
  await recoverLegacyScreenshotFalseFailure(ctx, projectId, makeLock());
  const after = db.getProject(projectId)!;
  // 旧的 3 条 log 必须保留
...
test("L5 截图再次失败：screenshot=undefined + warning + run 仍 succeeded + 项目仍 draft_saved", async (t) => {
  const db = await freshDb(t);
  const projectId = await setupStuckProject(db, makeStuckRun());
  ...
  const ctx = makeContext(db, browserMock.browser, browserMock.ensureBrowserHasBounds, () => undefined);
  const recovered = await recoverLegacyScreenshotFalseFailure(ctx, projectId, makeLock());
...
test("L2 业务真失败：任一 phase !== completed → 不恢复，原状态完全保留", async (t) => {
  ...
  const projectId = await setupStuckProject(db, run);
  const browserMock = makeBrowserMock();
  const ctx = makeContext(db, browserMock.browser, browserMock.ensureBrowserHasBounds, () => undefined);
  ...
  assert.equal(after.status, "blocked", "原项目状态必须保留");
...
test("L4 run 状态不是 failed：不恢复（running / succeeded / cancelled 都跳过）", async (t) => {
  ...
  for (const status of ["running", "succeeded", "cancelled"] as const) {
    const projectId = await setupStuckProject(db, { ...makeStuckRun(), status });
    const browserMock = makeBrowserMock();
    const ctx = makeContext(db, browserMock.browser, browserMock.ensureBrowserHasBounds, () => undefined);
    const recovered = await recoverLegacyScreenshotFalseFailure(ctx, projectId, makeLock());
...
test("L4 productId 缺失：不恢复（远程草稿未创建，没法切 draft_saved）", async (t) => {
  ...
  const project = db.createProject({ destination: "太原", days: 2, productForm: "privateTour" });
  db.updateProduct(project.id, project.product, "blocked");
  db.saveAutomation(project.id, makeStuckRun());
  const browserMock = makeBrowserMock();
  const ctx = makeContext(db, browserMock.browser, browserMock.ensureBrowserHasBounds, () => undefined);
  const recovered = await recoverLegacyScreenshotFalseFailure(ctx, project.id, makeLock());
  assert.equal(recovered, false);
});
...
function makeRealLock(projectId: string): { acquire: () => boolean; release: () => void; held: Set<string> } {
  const held = new Set<string>();
  return {
    held,
    acquire: () => (held.has(projectId) ? false : (held.add(projectId), true)),
    release: () => { held.delete(projectId); },
  };
}
...
test("L6 锁被预占时：acquire 失败 → 立即返回 false，不动 run/项目状态", async (t) => {
  const db = await freshDb(t);
  const projectId = await setupStuckProject(db, makeStuckRun());
  const browserMock = makeBrowserMock();
  const ctx = makeContext(db, browserMock.browser, browserMock.ensureBrowserHasBounds, () => undefined);
  const recovered = await recoverLegacyScreenshotFalseFailure(ctx, projectId, makePreHeldLock());
  assert.equal(recovered, false, "锁被占 → 直接返回 false，不进入恢复路径");
  const after = db.getProject(projectId)!;
  assert.equal(after.automation!.status, "failed", "锁被占期间不修改 run.status");
  assert.equal(after.status, "blocked", "锁被占期间不修改项目状态");
  assert.equal(browserMock.pageCalls, 0);
  assert.equal(browserMock.setVisibleCalls.length, 0);
});
...
test("L7 未命中/IO 异常时锁也必须被释放，retry 不会自锁", async (t) => {
  ...
  const projectA = dbA.createProject({ destination: "太原", days: 2, productForm: "privateTour" });
  dbA.updateProduct(projectA.id, projectA.product, "blocked");
  dbA.saveAutomation(projectA.id, makeStuckRun());
  const lockA = makeRealLock(projectA.id);
  const ctxA = makeContext(dbA, makeBrowserMock().browser, () => undefined, () => undefined);
  assert.equal(await recoverLegacyScreenshotFalseFailure(ctxA, projectA.id, lockA), false, "无 productId 不命中");
  assert.equal(lockA.held.has(projectA.id), false, "未命中必须立刻 release 锁");
  assert.equal(await recoverLegacyScreenshotFalseFailure(ctxA, projectA.id, lockA), false, "第二次仍可正常进入判定");

  // 子断言 B：page() 抛错 → 走 finally 释放；retry 不自锁
  const dbB = await freshDb(t);
  const projectIdB = await setupStuckProject(dbB, makeStuckRun());
  const lockB = makeRealLock(projectIdB);
  const failingBrowser = {
    setVisible: () => undefined,
    view: { getBounds: () => ({ width: 0, height: 0 }) },
    setBounds: () => undefined,
  ...
  assert.equal(await recoverLegacyScreenshotFalseFailure(ctxB, projectIdB, lockB), true, "page 抛错仍 succeeded");
  assert.equal(lockB.held.has(projectIdB), false, "finally 必须释放锁，retry 才不会自锁");
});
