...
const client = {
  agent:{get:async()=>structuredClone(snapshot),send:async(_id:string,content:string)=>{log('user',content);return emit();},
    ...
    approve:async()=>{ const approval={...snapshot.pendingApproval!,status:'approved' as const};log('approval','已确认方案',{approvalId:approval.id,approval});snapshot.pendingApproval=undefined;snapshot.run!.status='running';return emit();},
