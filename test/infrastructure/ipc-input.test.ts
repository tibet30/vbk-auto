...
test("产品写入与长流程通道拒绝空/超长/非法产品 ID", () => {
  for (const channel of ["ai:send", "planning:start", "planning:rerunMajorStage", "automation:start", "products:updateProductJson"]) {
...
test("AI 消息、自动化阶段和产品 JSON 有明确运行时上限", () => {
  ...
  assert.doesNotThrow(() => validateIpcArguments("workflowTasks:resume", ["task-1", "from_error"]));
  assert.doesNotThrow(() => validateIpcArguments("workflowTasks:resume", ["task-1", "from_start"]));
  assert.throws(() => validateIpcArguments("workflowTasks:resume", ["../bad task", "from_error"]), /field=localProductId/);
  assert.throws(() => validateIpcArguments("workflowTasks:resume", ["task-1", "invalid"]), /field=mode/);
  ...
  assert.doesNotThrow(() => validateIpcArguments("planning:rerunMajorStage", ["p-1", "completion"]));
  assert.throws(() => validateIpcArguments("planning:rerunMajorStage", ["p-1", "unknown"]), /field=stage/);
