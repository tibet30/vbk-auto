...
function openTask(db: VbkDatabase, projectId: string, label: string, type: "vbk" | "web" | "cost" | "image" = "vbk") {
  return db.addResearchTask(projectId, { label, type, detail: "历史遗留待处理" });
}

test("刷新待处理事项会清理已满足的用车 / 酒店 / 商业旧 task，保留未满足 POI", () => {
  const db = withDb();
  const project = db.createProject({ destination: "西安", days: 2, productForm: "privateTour" });
  db.updateProduct(project.id, {
    ...project.product,
    operations: {
      hotelTier: "当地5钻酒店/-38",
  ...
  const vehicleId = openTask(db, project.id, "核查用车资源组（按目的地 / 出行人数）");
  const hotelId = openTask(db, project.id, "核查酒店资源");
  const priceId = openTask(db, project.id, "核查成人价 / 儿童价 / 起订人数在 VBK 是否可发布");
  const inventoryId = openTask(db, project.id, "核查库存起止日期与每日配额在 VBK 是否生效");
  const packageId = openTask(db, project.id, "核查套餐名称与公开渠道展示一致", "web");
  const termsId = openTask(db, project.id, "核查费用包含 / 不包含 / 退改政策的运营成本口径", "cost");
  const poiId = openTask(db, project.id, "核查 回民街 的 VBK POI 映射");

  const result = refreshSatisfiedResearchTasks(db, project.id);
  ...
  const tasks = new Map(db.getProject(project.id)!.researchTasks.map((task) => [task.id, task]));
...
test("刷新待处理事项会清理产品行程中已保存有效 POI 的 canonical POI task", () => {
  ...
  const poiTaskId = openTask(db, project.id, "核查 西安明城墙 的 VBK POI 映射");

  assert.equal(isResearchTaskSatisfiedByProduct({
    label: "核查 西安明城墙 的 VBK POI 映射",
    type: "vbk",
  }, db.getProject(project.id)!.product), true);

  const result = refreshSatisfiedResearchTasks(db, project.id);
  assert.equal(result.updated, 1);
  assert.deepEqual(result.taskIds, [poiTaskId]);
  const task = db.getProject(project.id)!.researchTasks.find((item) => item.id === poiTaskId)!;
...
test("刷新待处理事项不会清理字段不完整的用车或非法酒店 task", () => {
  ...
  openTask(db, project.id, "核查用车资源组（按目的地 / 出行人数）");
  openTask(db, project.id, "核查酒店资源");

  const result = refreshSatisfiedResearchTasks(db, project.id);
  assert.equal(result.updated, 0);
  assert.ok(db.getProject(project.id)!.researchTasks.every((task) => task.state === "researching"));
});
...
test("刷新待处理事项不会因 POI 名称含酒店 / 用车字样而误清理 POI 任务", () => {
  ...
  const hotelPoiId = openTask(db, project.id, "核查 西安民宿客栈街 的 VBK POI 映射");
  const legacyPoiId = openTask(db, project.id, "待核查景点 接送中心广场 的 VBK POI");
  const coverId = openTask(db, project.id, "获取产品封面图", "image");
...
test("刷新待处理事项幂等：第二次不重复写 evidence，也不改已 confirmed task", () => {
  ...
  const vehicleId = openTask(db, project.id, "核查用车资源组");

  const first = refreshSatisfiedResearchTasks(db, project.id);
  const afterFirst = db.getProject(project.id)!.researchTasks.find((task) => task.id === vehicleId)!;
  const second = refreshSatisfiedResearchTasks(db, project.id);
  const afterSecond = db.getProject(project.id)!.researchTasks.find((task) => task.id === vehicleId)!;
...
function openImageTask(db: VbkDatabase, projectId: string, label = "获取产品封面图") {
  return db.addResearchTask(projectId, { label, type: "image", detail: "POI 云冈石窟；描述：横版；最低质量 3。" });
}

test("image 类型封面任务在产品含完整 ctripLibrary 封面（持久化可选元数据齐全）时被确认", () => {
  const db = withDb();
  const project = db.createProject({ destination: "云冈石窟", days: 1, productForm: "privateTour" });
  const imageTaskId = openImageTask(db, project.id);

  // 第一轮：产品还没有封面 -> 不应被满足
  const firstPass = refreshSatisfiedResearchTasks(db, project.id);
  assert.equal(firstPass.updated, 0);
  const beforeCover = db.getProject(project.id)!.researchTasks.find((task) => task.id === imageTaskId)!;
  ...
  // 第二轮：应被满足
  const secondPass = refreshSatisfiedResearchTasks(db, project.id);
  assert.equal(secondPass.updated, 1);
  assert.deepEqual(secondPass.taskIds, [imageTaskId]);
  const afterCover = db.getProject(project.id)!.researchTasks.find((task) => task.id === imageTaskId)!;
...
test("image 类型封面任务在产品含完整 manualUpload 封面（缺省任何旧字段也不破坏）时被确认", () => {
  ...
  const task = db.getProject(project.id)!.researchTasks.find((item) => item.id === imageTaskId)!;
  assert.equal(task.state, "confirmed");
  assert.equal(task.status, "succeeded");
...
test("封面任务确认后的二次刷新幂等：不再重复写 evidence，也不再次计数", () => {
  ...
  const first = refreshSatisfiedResearchTasks(db, project.id);
  assert.equal(first.updated, 1);
  const afterFirst = db.getProject(project.id)!.researchTasks.find((task) => task.id === imageTaskId)!;

  const second = refreshSatisfiedResearchTasks(db, project.id);
  const afterSecond = db.getProject(project.id)!.researchTasks.find((task) => task.id === imageTaskId)!;
...
test("封面任务已 confirmed / resolved 时不再被二次覆盖（跳过收敛）", () => {
  ...
  // 提前手动置为 confirmed
  db.markResearchTasksSatisfied(project.id, [imageTaskId]);
  const evidenceBefore = db.getProject(project.id)!.researchTasks.find((task) => task.id === imageTaskId)!.evidence;

  const result = refreshSatisfiedResearchTasks(db, project.id);
  assert.equal(result.updated, 0);
  const after = db.getProject(project.id)!.researchTasks.find((task) => task.id === imageTaskId)!;
