...
test("poi:suggest IPC 调试日志覆盖开始、详情、成功、空结果和失败分支", () => {
  ...
  assert.match(handler, /rawPayload: stringifyPoiPayloadForLog\(result\.rawPayload\)/);
...
test("规划 runtime 通过 resolvePlanningPoiAutoSelection 接线，不再暴露已移除的手动选择入口", () => {
  const runtime = readFileSync("src/main/planning/runtime.ts", "utf8");
  assert.match(runtime, /resolvePlanningPoiAutoSelection\(/);
  assert.match(runtime, /async resolvePoiSelection\(/);
